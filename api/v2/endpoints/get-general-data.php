...
if (empty($error_code)) {
    ...
    if(!empty($data['notificaitons_count']))
    {
        $count_notifications = Wo_CountNotifications(array(
            'unread' => true,
            'remove_notification' => array('requested_to_join_group', 'interested_event', 'going_event', 'invited_event', 'forum_reply', 'admin_notification')
        ));
        $response_data['count'] = $count_notifications;
    }
    
    if (!empty($data['friend_requests'])) {
    	$final_friend_requests = array();
    	$friend_requests = Wo_GetFriendRequests();
        ...
        $response_data['new_friend_requests_count'] = Wo_CountFriendRequests();
    }
    ...
    if (!empty($data['group_chat_requests'])) {
        ...
        if (!empty($group_chat_requests)) {
            foreach ($group_chat_requests as $key => $group_chat_request) {
                ...
                $user = Wo_UserData($group_chat_request->group_tab['user_id']);
                unset($group_chat_request->group_tab['messages']);
                $group_chat_request->group_tab['invited_user_name'] = $user['name'];
    ...
    if (!empty($data['count_new_messages'])) {
		...
		$chat_msg = Wo_CountMessages(array('new' => true), 'interval');
        ...
        $response_data['count'] = (int)$count_notifications;
        $response_data['count_new_messages'] = $chat_msg + count($chat_groups);
    }
