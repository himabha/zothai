...
if (!empty($_POST['type']) && in_array($_POST['type'], $required_fields)) 
{
  	...
  	if($_POST['type']=='invitations')
    {
        ...
        if (empty($error_code)) 
        {
            $group_data = Wo_GetChatGroupDataAll($id);
            $response_data = array(
                'api_status' => 200,
                'data' => $group_data
            );
                ...
                if ($group_tab['user_id'] == $wo['user']['id']) {
                    foreach ($users as $key => $user) {
                        if (!Wo_IsGChatMemeberExists($id, $user)) {
                            @mysqli_query($sqlConnect, "INSERT INTO " . T_GROUP_CHAT_USERS . " (`id`,`user_id`,`group_id`,`last_seen`,`active`) VALUES (null,$user,$id,'0','0')");
                        }
                    }
                    $response_data = array(
                        'api_status' => 200,
