...
function add_social_post_hub_categories($category_name,$parent_id,$position)
{
    global $sqlConnect;
    $level=2;
    if($parent_id ==0)
    ...
    $query_three = mysqli_query($sqlConnect, "INSERT INTO " . T_categories . " (`name`,`position`, `parent_id`, `created_by`,`level`) VALUES('{$category_name}','{$position}', '{$parent_id}', '1', '$level')");
    if ($query_three) {
        return true;
    } else {
        return false;
...
function Wo_GcategoriesById($category_id) {
    ...
    {
        $parent_id = $fetched_data['parent_id'];
        $data['category'] =$fetched_data['name'];
...
function Wo_GetLocationById($id) {
    global $sqlConnect;
    $data  = array();
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_location. " where id=".$id);
    while ($fetched_data = mysqli_fetch_assoc($query)) 
    {
        $id = $fetched_data['id'];
        $data =$fetched_data;
...
function Wo_GetLocations($type) {
    ...
    $data  = array();
    $i=0;
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_location. " where type=".$type);
    while ($fetched_data = mysqli_fetch_assoc($query)) 
    {
        $id = $fetched_data['id'];
        $data[$i] =$fetched_data;
