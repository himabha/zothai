...
function Wo_GetCustomMenus() {
	
    global $sqlConnect;
    $data          = array();
    $query_one     = "SELECT * FROM Wo_CustomMenu ORDER BY `id` DESC";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
        $data[] = Wo_GetCustomMenu($fetched_data['id']);
    }
	/* echo '<pre>';
	print_r($data);
	die('hii'); */
...
function Wo_GetCustomMenu($page_name) {
    global $sqlConnect;
    if (empty($page_name)) {
        return false;
    }
    ...
    $query_one     = "SELECT * FROM Wo_CustomMenu WHERE `id` = {$page_name}";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
...
function Wo_UpdateMenuPageData($id, $update_data) {
    global $wo, $sqlConnect, $cache;
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $impload   = implode(', ', $update);
    $query_one = "UPDATE Wo_CustomMenu SET {$impload} WHERE `id` = {$id} ";
    $query     = mysqli_query($sqlConnect, $query_one);
    if ($query) {
        return true;
    }
            ...
            if (empty($get_session_data) || !empty($get_session_data)) {
                ...
                if (!empty($to_data['android_n_device_id']) || !empty($to_data['ios_n_device_id']) || !empty($to_data['web_device_id'])) {
                    ...
                    if (!empty($sql_get_notification_for_push['type'])) {
                        ...
                        if ($sql_get_notification_for_push['type'] == "friend_request") {
                            $sql_get_notification_for_push['type_text'] .= $wo['lang']['friend_request_sent'];
                        }
                        ...
                        if ($sql_get_notification_for_push['type'] == 'accepted_friend_request') 
                        {
                            //if ($wo['config']['connectivitySystem'] == 1) {
                                $sql_get_notification_for_push['type_text'] = $wo['lang']['accepted_friend_request'];
                            // } else {
...
function Wo_GetUsersByName($name = '', $friends = false, $limit = 25) {
    ...
    $sql   = "SELECT `user_id` FROM " . T_USERS . " WHERE `user_id` <> {$user} AND (`username` LIKE '%$name%' OR `first_name` LIKE '%$name%' OR `last_name` LIKE '%$name%') {$sub_sql} $limit_text";
    $query = mysqli_query($sqlConnect, $sql);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        $data[] = Wo_UserData($fetched_data['user_id']);
    }
...
function Wo_DeletePendingPaymentVerificationRequest() {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $user  = Wo_Secure($wo['user']['id']);
    $sql   = "DELETE FROM " . T_VERIFICATION_REQUESTS . " WHERE `payment_status` = 0 and `user_id` = {$user}";
    return mysqli_query($sqlConnect, $sql);
}
...
// request notification user group data
function Wo_GetChatGroupDataAll($id = false) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false || empty($id) || !is_numeric($id) || $id < 1) {
        return false;
    }
    $user    = $wo['user']['id'] != $id ? Wo_Secure($id) : Wo_Secure($wo['user']['id']);
    $data    = array();
    $sql   = "SELECT * FROM " . T_GROUP_CHAT . " 
                WHERE `group_id` IN 
                   (SELECT `group_id` FROM Wo_GroupChatUsers  WHERE `user_id` = {$user} AND `active` = '0')";
    $query = mysqli_query($sqlConnect, $sql);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        $fetched_data['owner_data']   = Wo_UserData($fetched_data['user_id']);
        $fetched_data['group_id']     = $fetched_data['group_id'];
        $fetched_data['group_name']   = $fetched_data['group_name'];
        $fetched_data['avatar']       = Wo_GetMedia($fetched_data['avatar']);
        $fetched_data['last_seen']    = Wo_CheckLastGroupAction();
        $data[]                       = $fetched_data;
    }
...
function Wo_CusSuspendUser($user_id,$active)
{
    ...
    if ($posts) {
        $query_one = "";
			if ($active == 1 ){
				$active = 2;
				$query_one = "UPDATE " . T_USERS . " SET `active` = '{$active}' WHERE `user_id` = {$user_id} ";
			}else{
				$active = 1;
				$query_one = "UPDATE " . T_USERS . " SET `active` = '{$active}' WHERE `user_id` = {$user_id} ";
			}
