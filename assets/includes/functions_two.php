...
function Wo_GetVerifications() {
    global $wo, $sqlConnect;
    $data      = array();
    $query_one = " SELECT * FROM " . T_VERIFICATION_REQUESTS . " where payment_status = 1 ORDER BY `id` DESC";
    $sql       = mysqli_query($sqlConnect, $query_one);
    while ($fetched_data = mysqli_fetch_assoc($sql)) {
...
function Wo_CountOnlineUsers() {
    ...
    $time         = time() - 60;
    $user_id      = Wo_Secure($wo['user']['user_id']);
    $query        = mysqli_query($sqlConnect, "SELECT COUNT(`user_id`) AS `online` FROM " . T_USERS . " WHERE `user_id` IN (SELECT `following_id` FROM " . Wo_friends . " WHERE `follower_id`= {$user_id} AND `following_id` <> {$user_id} AND `active` = '1') AND `lastseen` > {$time} AND `active` = '1' AND `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}') ORDER BY `lastseen` DESC");
...
function Wo_GetChatUsers($type) {
    ...
    $time       = time() - 60;
    $user_id    = Wo_Secure($wo['user']['user_id']);
    $query_text = "SELECT `user_id` FROM " . T_USERS . " WHERE `user_id` IN (SELECT `following_id` FROM " . Wo_friends . " WHERE `follower_id` = {$user_id} AND `following_id` <> {$user_id} AND `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}') AND `active` = '1')";
    if ($type == 'online') {
...
function Wo_ChatSearchUsers($search_query = '') {
    ...
    $user_id      = Wo_Secure($wo['user']['user_id']);
    $query_one    = "SELECT `user_id` FROM " . T_USERS . " WHERE (`user_id` IN (SELECT `following_id` FROM " . Wo_friends . " WHERE `follower_id` = {$user_id} AND `following_id` <> {$user_id} AND `active` = '1') AND `active` = '1'";
...
function Wo_GetUserGroupRequestsWithOffset($request = array()) {
    global $wo, $sqlConnect;
    if (empty($request) || !is_array($request) || empty($request['user_id'])) {
        return false;
    }
    $data      = array();
    $user_id  = Wo_Secure($request['user_id']);
    $limit_query = "";
    $offset_query = "";
    if (!empty($request['limit'])) {
        $limit_query = " LIMIT ".Wo_Secure($request['limit']);
    ...
    $query_one = " SELECT `id`,`group_id` FROM " . T_GROUP_MEMBERS . " WHERE `user_id` = {$user_id} {$offset_query} AND `active` = '0' ORDER BY `id` DESC ".$limit_query;
    $sql       = mysqli_query($sqlConnect, $query_one);
    while ($fetched_data = mysqli_fetch_assoc($sql)) {
        $new_data = array();
        $new_data['id'] = $fetched_data['id'];
        $new_data['group_data'] = Wo_GroupData($fetched_data['group_id']);
        $data[] = $new_data;
    }
    return $data;
}
...
function Wo_GetAlbumPhotos($post_id) {
    ...
    $data      = array();
    $post_id   = Wo_Secure($post_id);
    $query_one = "SELECT `id`,`image`,`post_id`,`parent_id` FROM " . T_ALBUMS_MEDIA . " WHERE `post_id` = {$post_id} ORDER BY `id` ASC";
    $sql       = mysqli_query($sqlConnect, $query_one);

    $query_2 = "SELECT `id`,`image`,`post_id`,`parent_id` FROM " . T_ALBUMS_MEDIA . " WHERE `parent_id` = {$post_id} ORDER BY `id` ASC";
...
function Wo_ConfirmUserOtp($user_id, $code) {
    global $sqlConnect;
    $user_id = Wo_Secure($user_id);
    $code    = Wo_Secure($code);
    if (!is_numeric($code) || $code <= 0) {
   ...
   /*  if (!is_numeric($user_id) || $user_id <= 0) {
    ...
    $query   = mysqli_query($sqlConnect, " SELECT COUNT(`user_id`)  FROM " . T_USERS . "  WHERE `otp_code` = '{$code}' AND `email` = '{$user_id}' AND `active` = '0'");
    $result  = Wo_Sql_Result($query, 0);
    if ($result == 1) {
		return true;
        /* $email_code = md5(rand(1111, 9999) . time());
...
function Wo_PayPal_For_group_board($type = 'week', $type2 = '', $plan_amount = 0) 
{
    global $wo;
    // if ($wo['config']['pro'] == 0 || $wo['config']['paypal'] == 'no') {
    //     return false;
	...
	if($type2 == "package") {
		$product  = $wo['config']['siteName'] . ' purchase package.';
	} else {	
		$product  = $wo['config']['siteName'] . ' Post Move to Board.';
	}
    ...
    /*if (!empty($type2) && $type2 == 'upgrade') {
        ...
        $redirectUrls->setReturnUrl($wo['config']['site_url'] . "/requests.php?f=payment&success=true&pro_type={$pro_type}&payment_type={$type2}")->setCancelUrl($wo['config']['site_url'] . '/requests.php?f=payment&success=false');
    //}
    $payment = new Payment();
    $payment->setIntent('sale')->setPayer($payer)->setRedirectUrls($redirectUrls)->setTransactions(array(
        $transaction
