...
function Wo_SeoLink($query = '') {
    ...
    if ($wo['config']['seoLink'] == 1) {
        $query = preg_replace(array(
            ...
            '/^index\.php\?link1=timeline&u=([A-Za-z0-9_]+)&type=([A-Za-z0-9_]+)&page=([A-Za-z0-9_-]+)$/i',
            // '/^index\.php\?link1=timeline&u=([\p{L}\u0041-\u007A\u0400-\u04FF\u0900-\u097F\u0E00-\u0E7F\u4E00-\u9FFF]+)&type=([A-Za-z0-9_]+)&page=([A-Za-z0-9_-]+)$/i',
            ...
            '/^index\.php\?link1=welcome$/i',
             '/^index\.php\?link1=open-space$/i',
              '/^index\.php\?link1=login?last_url=(.*)$/i',
        ), array(
            $config['site_url'] . '/edit_fund/$1',
            $config['site_url'] . '/show_fund/$1',
            $config['site_url'] . '/$1/$2&id=$3',
            $config['site_url'] . '/$1?page=$3',
            ...
            $config['site_url'] . '/$1',
            $config['site_url'],
            $config['site_url'] . '/open-space',
            $config['site_url'] . '/login/?last_url=$1',
        ), $query);
    } else {
