...
if ($f == 'posts') {
    ...
    if ($s == 'insert_new_post') {
        ...
        if (isset($_FILES['postVideo']['name']) && empty($mediaFilename)) {
            ...
            if ($_FILES['postVideo']['size'] > $wo['config']['maxUpload']) {
				...
				$video = $_FILES["postVideo"]["tmp_name"];
				$resolution = '350k';
        ...
        if (isset($_FILES['postPhotos']['name'])) {
            ...
            for ($i = 0; $i < count($_FILES['postPhotos']['name'])-1; $i++) {
        ...
        if (empty($errors) && $invalid_file == false) {
           ...
           // echo "<pre>";print_r($post_data);die;
            ...
            if ($id) {
                ...
                if (isset($_FILES['postPhotos']['name'])) {
                    if (count($_FILES['postPhotos']['name']) > 0) {												/*----check image ----*/						if(count($_FILES['postPhotos']['name']) >= 5)						{							$arr = 4;						}						else						{							$arr = count($_FILES['postPhotos']['name']);						}
                        for ($i = 0; $i < $arr; $i++) {
                            $fileInfo = array(
    ...
    if ($s == 'edit_post') {
        ...
        if (!empty($_POST['post_id'])) {
			...
			if($_POST['post_Date'] != "" && $_POST['post_Date'] != "undefined") {
				$postdate = strtotime($_POST['post_Date']);
			} else {
				$postdate = $post->time;
			}
            ...
            if (!empty($updatePost)) {
				...
				if ($_FILES['postPhotos']['name'][0] != "" && !empty($_FILES)) {
                    if (count($_FILES['postPhotos']['name']) > 0) 
					{	
						if($postFile != "") 
						{							
							$query = mysqli_query($sqlConnect, "UPDATE " . T_POSTS . " SET `multi_image` = '1',`postFile` = '', `postFileName` = '' WHERE `id` = {$id}");
							
							$media_album                   = Wo_RegisterAlbumMedia($id,$postFile);
							$post_data['multi_image']      = 0;
							$post_data['multi_image_post'] = 1;
							$post_data['album_name']       = '';
							$post_data['postFile']         = $postFile;
							$post_data['postText']         = $_POST['text'];
							$post_data['postFileName']     = $postFileName;
							$post_data['active']           = 1;
							$new_id                        = Wo_RegisterPost($post_data);
							$media_album                   = Wo_RegisterAlbumMedia($new_id,$postFile,$id);
						} else {
							$query = mysqli_query($sqlConnect, "UPDATE " . T_POSTS . " SET `multi_image` = '1' WHERE `id` = {$id}");
						}
						
						if(count($_FILES['postPhotos']['name']) >= 5) {							
							$arr = 4;						
						} else {							
							$arr = count($_FILES['postPhotos']['name']);						
						}
				...
				$wo['story']['time'] = $postdate;
                Wo_CleanCache();
				
				$returnData = array();
				$fetched_data = Wo_GetAlbumPhotos($id);
				...
				if (!empty($fetched_data)) {
					...
					if(count($fetched_data) == 1){
						foreach ($fetched_data as $id => $photo) {
							...
							$mediaUpdatedHtml .= '<img src="'.Wo_GetMedia($photo['image_org']).'" alt="image" class="image-file pointer" onclick="'.$multi_image_function.'"> ';
						}
					} else if(count($fetched_data) == 2){
						foreach ($fetched_data as $id => $photo) {
							$multi_image_function = "Wo_OpenAlbumLightBox(" . $photo['id'] . ", \"album\");";
							if (!empty($photo['parent_id'])) {
								$multi_image_function = "Wo_OpenLightBox(" . $photo['parent_id'] . ", \"album\");";
					...
					} else if(count($fetched_data) == 4){
						$mediaUpdatedHtml .= '
						<div class="wo_adaptive_media_4">';
						foreach ($fetched_data as $id => $photo) {
							$multi_image_function = "Wo_OpenAlbumLightBox(" . $photo['id'] . ", \"album\");";
							if (!empty($photo['parent_id'])) {
								$multi_image_function = "Wo_OpenLightBox(" . $photo['parent_id'] . ", \"album\");";
					...
					} else if(count($fetched_data) == 5){
						$mediaUpdatedHtml .= '
						<div class="wo_adaptive_media_5">';
						foreach ($fetched_data as $id => $photo) {
							$multi_image_function = "Wo_OpenAlbumLightBox(" . $photo['id'] . ", \"album\");";
							if (!empty($photo['parent_id'])) {
								$multi_image_function = "Wo_OpenLightBox(" . $photo['parent_id'] . ", \"album\");";
						...
						$mediaUpdatedHtml .= '
						<div style="text-align:center;width: 100%;" class="album-image"';
						foreach ($fetched_data as $id => $photo) {
							$multi_image_function = "Wo_OpenAlbumLightBox(" . $photo['id'] . ", \"album\");";
							if (!empty($photo['parent_id'])) {
								$multi_image_function = "Wo_OpenLightBox(" . $photo['parent_id'] . ", \"album\");";
                ...
                $data = array(
                    'status' => 200,
                    'postdate' => Wo_Time_Elapsed_String($postdate),
                    'postdatetitle' => date('c',$postdate),
                    'html' => $updatePost,
					'mediaUpdatedHtml' => $mediaUpdatedHtml
                );
                if (Wo_CanSenEmails()) {
                    $data['can_send'] = 1;
                }
	...
	if ($s == 'delete_post_image') {
		if (!empty($_POST['image_id']) && !empty($_POST['post_id']) && !empty($_POST['parent_id']) && !empty($_POST['uploade_path'])) {
            if (Wo_DeletePostImage($_POST['image_id'],$_POST['post_id'],$_POST['parent_id'],$_POST['uploade_path']) === true) {
                $data = array(
                    'status' => 200,
                    'post_count' => $user_data['details']['post_count']
                );
	...
	if ($s == 'delete_post_file') {
		if (!empty($_POST['post_id'])) 
		{
			$post_id = $_POST['post_id'];
			$query   = mysqli_query($sqlConnect, "SELECT `id`, `postFile` FROM " . T_POSTS . " WHERE `id` = {$post_id}");
			...
			if($is_me > 0) {
				...
				$query = mysqli_query($sqlConnect, "UPDATE " . T_POSTS . " SET `postFile` = '', `postFileName` = '' WHERE `id` = {$post_id}");
