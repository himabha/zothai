...
if ($f == "update_privacy_settings") {
    if (isset($_POST['user_id']) && Wo_CheckSession($hash_id) === true) {
        ...
        if (!empty($_POST['profile_privacy'])) {
            if (in_array($_POST['profile_privacy'], $array_2)) {
        ...
        if (!empty($_POST['friend_privacy'])) {
		 ...
		 if (!empty($_POST['post_page_privacy'])) {
            if (in_array($_POST['post_page_privacy'], $array_four)) {
                $post_page_privacy = $_POST['post_page_privacy'];
            }
        }
		 if (!empty($_POST['for_yous_privacy'])) {
            if (in_array($_POST['for_yous_privacy'], $array_five)) {
                $for_yous_privacy = $_POST['for_yous_privacy'];
            }
        }
		if (!empty($_POST['followers_privacy'])) {
            if (in_array($_POST['followers_privacy'], $array_six)) {
        ...
        $Update_data = array(
			...
			'post_page_privacy' => $post_page_privacy,
			'for_yous_privacy' => $for_yous_privacy,
			'followers_privacy' => $followers_privacy,
