...
function Wo_RegisterCommentContent(text, post_id, user_id, event, page_id, type) {
  text = $('[id=post-content-' + post_id + ']').find('.comment-textarea').val();
  console.log(text);
  if(event.keyCode == 13 && event.shiftKey == 0 && recording_node == "comm") {
    Wo_stopRecording(); 
    if (recorder) { 
      recorder.exportWAV(function(blob){
        var comment_src_image = $('#post-content-' + post_id).find('#comment_src_image');
        var comment_image = '';
        ...
        Wo_InsertCommentContent(dataForm,post_id);
    }
  }
}
function Wo_RegisterComment2Content(post_id, user_id, page_id, type) {
  text = $('[id=post-content-' + post_id + ']').find('.comment-textarea').val();
...
function Wo_InsertCommentContent(dataForm,post_id){
    if (!dataForm) { return false;}
    post_wrapper = $('[id=post-content-' + post_id + ']');
    comment_textarea = post_wrapper.find('.post-comments');
    comment_btn = comment_textarea.find('.emo-comment');
    textarea_wrapper = comment_textarea.find('.textarea');
    comment_list = post_wrapper.find('.comments-list');   
    ...
    post_wrapper.find('#wo_comment_combo_content .ball-pulse').fadeIn(100);
    $.ajax({
        url: Wo_Ajax_Requests_File() + '?f=posts&s=register_comment&hash=' + $('.main_session').val(),
        type:       'POST',
        cache:       false,
      ...
      $('.wo_comment_combo_content' + post_id).removeClass('comment-toggle');
      if(data.status == 200) {
        Wo_CleanRecordNodes();
        post_wrapper.find('.post-footer .comment-container:last-child').after(data.html);
        post_wrapper.find('.comments-list-lightbox .comment-container:first').before(data.html);
      ...
      $('#post-content-'+ post_id).find('.comment-image-con').empty().addClass('hidden');
      $('#post-content-'+ post_id).find('#comment_src_image').val('');
      post_wrapper.find('#wo_comment_combo_content .ball-pulse').fadeOut(100);
      if (data.can_send == 1) {
        Wo_SendMessages();
      }
    });
