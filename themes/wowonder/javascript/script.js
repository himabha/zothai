...
function Wo_OpenFriendsRequestsMenu() {
  requests_container = $('.requests-container');
  requests_List = $('#requests-list');
  Wo_progressIconLoader(requests_container.find('.requests-loading'));
  $.get(Wo_Ajax_Requests_File(), {
    f: 'get_friend_requests'
  }, function (data) {
    if(data.status == 200) {
      if(data.html.length == 0) {
        requests_List.html('<div class="empty_state"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M14 14.252v2.09A6 6 0 0 0 6 22l-2-.001a8 8 0 0 1 10-7.748zM12 13c-3.315 0-6-2.685-6-6s2.685-6 6-6 6 2.685 6 6-2.685 6-6 6zm0-2c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm7 6.586l2.121-2.122 1.415 1.415L20.414 19l2.122 2.121-1.415 1.415L19 20.414l-2.121 2.122-1.415-1.415L17.586 19l-2.122-2.121 1.415-1.415L19 17.586z" fill="currentColor"/></svg>' + data.message + '</div>');
...
function Wo_ShowCommentsContent(post_id) {
  $('#post-comments-content-' + post_id).toggleClass('hidden');
}
...
// save edited post
function Wo_EditPost(post_id,uniqueCode) {
	var post            = $('#post-' + post_id);
	var freeToBoardPost = $('#post-content-' + post_id);
	
	$('#posttime-' + post_id).removeAttr("title");
	var edit_box = $('#post-' + post_id).find('#edit-post');
	var post_Date = $('.edit-postdate-' + post_id).val();
	var edit_textarea  = post.find('.edit-textarea-' + post_id + ' textarea');
	var edit_postTitle = post.find('.edit-textarea-' + post_id + ' .editPostTitle');
	var text      = edit_textarea.val();
	var postTitle = edit_postTitle.val();
	
	var post_text       = post.find('.post-description p');
	var free_post_text  = freeToBoardPost.find('.post-description p');
	
	var post_title_text = post.find('.post-description h4');
	var free_title_text = freeToBoardPost.find('.post-description h4');
	
	var post_title_image = post.find('#fullsizeimg');
	var free_title_image = freeToBoardPost.find('#fullsizeimg');
	
	var type = post.attr('data-post-type');
	if (type == 'share') {
		var post_text      = post.find('.post-description .edited_text');
		var free_post_text = freeToBoardPost.find('.post-description .edited_text');
	} else {
		var post_text      = post.find('.post-description p');
		var free_post_text = freeToBoardPost.find('.post-description p');
	}
	...
	var formData   = new FormData($("#editPostData"+uniqueCode)[0]);
	...
	.done(function(data) {
		if (data.status == 200) {
			...
			// free_post_text.html(data.html);
			
			// post_title_text.html(postTitle);
			// free_title_text.html(postTitle);
			
			// console.log(post_title_image);
			// console.log(data.mediaUpdatedHtml);
			
			// post_title_image.html(data.mediaUpdatedHtml);
			// free_title_image.html(data.mediaUpdatedHtml);
			
			$('#posttime-' + post_id).text(data.postdate);
			$('#posttime-' + post_id).attr("title",data.postdatetitle);
...
// delete post image
function DeleteImageFromDb(image_id,post_id,parent_id,uploade_path,uniqueCode){
	if(confirm('Are you sure you want to delete this image?')){
		$.post(Wo_Ajax_Requests_File() + '?f=posts&s=delete_post_image', {
			image_id: image_id,
			post_id: post_id,
			parent_id: parent_id,
			uploade_path: uploade_path
		}, function (data) {
			if(data.status == 200) {
				$('#edit_image_to_'+uniqueCode).remove();
			}
		});
	}
}

function DeletePostFileFromDb(post_id,uniqueCode){
	if(confirm('Are you sure you want to delete this image?')){
		$.post(Wo_Ajax_Requests_File() + '?f=posts&s=delete_post_file', {
			post_id: post_id
		}, function (data) {
			if(data.status == 200) {
				$('#edit_image_to_'+uniqueCode).remove();
...
function Wo_PinPostContent(post_id, id, type) {
  console.log(post_id);
...
function Wo_AcceptFriendRequest(user_id) {
  var main_container = $('.user-follow-request-' + user_id);
  var follow_main_container = main_container.find('#accept-' + user_id);
  Wo_progressIconLoader(follow_main_container);
  $.get(Wo_Ajax_Requests_File(), {
    f: 'accept_friend_request',
    following_id: user_id
  }, function (data) {
    if(data.status == 200) {
      main_container.find('.accept-btns').html(data.html);
...
function Wo_ShowCommentComboContent(post_id){
  comment_combo_wrapper = $('.wo_comment_combo_content' + post_id);
  comment_combo_wrapper.addClass('comment-toggle');
  comment_combo_wrapper.find('textarea').focus();
}
